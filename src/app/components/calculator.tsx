...
export default function Calculator() {
  ...
  return (
    <div className="max-w-sm mx-auto mt-10 p-6 bg-gray-900 rounded-2xl shadow-2xl">
      {/* History Display */}
      <div className="mb-2 h-6 text-sm text-gray-400 text-right overflow-hidden">{history || ' '}</div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {buttons.map((button, index) => (
          <button key={index} onClick={button.onClick} className={`${button.className} ${button.colSpan === 2 ? 'col-span-2' : ''}`}>
            {button.label}
          </button>
        ))}
      </div>
